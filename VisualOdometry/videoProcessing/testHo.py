...
class CameraPoses():
    ...
    def __init__(self, data_dir, skip_frames, intrinsic):
        ...
        FLANN_INDEX_LSH = 6
        index_params = dict(algorithm=FLANN_INDEX_LSH, table_number=7, key_size=12, multi_probe_level=1)
    ...
    def decomp_essential_mat(self, E, q1, q2):
        ...
        # if positives array has inf values, replace them with 0
    ...
    def decomp_essential_mat_old(self, E, q1, q2):
        ...
        if relative_scales.count(float('inf')) > 0:
              relative_scales = [1 if x == float('inf') else x for x in relative_scales]
...
cap = cv2.VideoCapture('vods/vidcompsle.mp4')
...
while(cap.isOpened()):
    ...
    if process_frames and ret and frame_counter % 20 == 0:
...
key_frames_indices = np.linspace(0, len(camera_pose_poses) - 1, dtype=int)
colors = cycle("rgb")
